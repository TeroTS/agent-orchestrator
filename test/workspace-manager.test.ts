...
describe("ensureWorkspace", () => {
  it("creates a deterministic workspace directory and runs after_create once", async () => {
    ...
    await expect(readFile(join(created.path, ".created-marker"), "utf8")).resolves.toBe("created");
    ...
    await expect(readFile(join(reused.path, ".created-marker"), "utf8")).resolves.toBe("created");
  });
...
describe("prepareWorkspaceForRun", () => {
  it("removes temporary artifacts and runs before_run on every attempt", async () => {
    ...
    await expect(stat(join(workspace.path, ".elixir_ls"))).rejects.toBeDefined();
    await expect(readFile(join(workspace.path, ".before-run"), "utf8")).resolves.toBe("ready");
  });
...
describe("validateWorkspacePathWithinRoot", () => {
  it("accepts workspace paths inside the workspace root", () => {
    ...
    expect(() => validateWorkspacePathWithinRoot(root, workspacePath)).not.toThrow();
  });

  it("rejects workspace paths outside the workspace root", () => {
    try {
      validateWorkspacePathWithinRoot("/tmp/symphony", "/tmp/somewhere-else/ABC-1");
